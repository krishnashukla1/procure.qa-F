...
const Products = () => {
  ...
  const fetchDropdownData = async () => {
    try {
      ...
      console.log("📦 Categories response:", catRes.data);
      console.log(
        "📦 Suppliers response FULL:",
        JSON.stringify(supRes.data, null, 2)
      ); // NEW: Full for debug
      ...
      console.log("📦 Mapped suppliers before final filter:", supData.length);
      console.log(
        "📦 Valid suppliers after filter:",
        validSuppliers.length,
        validSuppliers.slice(0, 2)
      );

      if (validSuppliers.length === 0) {
        console.warn("⚠️ No valid suppliers found - check backend/DB");
        toast.error(
          "No valid suppliers available. Please add suppliers first or check console for details."
        );
      } else {
        toast.success(`Loaded ${validSuppliers.length} suppliers!`); // Optional positive feedback
      }
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      const res = await apiCalls.bulkUploadProducts(
        bulkSupplierId,
        formDataUpload
      );
          ...
          <div className="space-y-6">
            <div>
              ...
              <select
                value={bulkSupplierId}
                onChange={(e) => setBulkSupplierId(e.target.value)}
                className="cursor-pointer w-full border rounded-lg p-3"
              >
                <option value="">Select Supplier</option>
                {suppliers.map((sup) => (
                  <option key={sup._id} value={sup._id}>
            ...
            <div className="cursor-pointer border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
              <Upload className=" w-12 h-12 text-gray-400 mx-auto mb-4" />
              ...
              <ul className="text-sm text-blue-800 space-y-2">
                ...
                <li className="flex items-start gap-2">
                  ...
                  <span>
                    ...
                    <span className="font-medium text-blue-900">
                      {" "}
                      Product Name, Item Code* (must be unique), Unit*, Group,
                      Brand, Description
                    </span>
                  </span>
                </li>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={handleBulkUpload}
                disabled={!file || !bulkSupplierId}
                className="cursor-pointer flex-1 bg-green-600 hover:bg-green-700 text-white px-4 py-3 rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Upload Products
              </button>
            </div>
          ...
          <form onSubmit={handleCreateOrUpdate} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                  <option value="">
                    Select Subcategory (after choosing category)
                  </option>
                  {subcategories.map((sub) => (
                    <option key={sub._id} value={sub._id}>
                      {sub.name}
